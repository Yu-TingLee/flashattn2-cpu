...
int main(int argc, char** argv) {
    ...
    const std::string out_dir  = "outputs/naive_causal_cpp_" + opt;
    ...
    if (std::filesystem::exists(log_path)) {
        ...
        while (std::getline(fin, line)) {
            ...
            if (p.size() >= 5 && p[4] == "1") {
                try { done.insert({std::stoi(p[0]), std::stoi(p[1]), std::stoi(p[2])}); }
                catch (...) {}
            }
        }
    } else {
    ...
    for (int i = 0; i < num_testsets; ++i) {
        ...
        auto O = naive_attention_causal(Q, Kt, V, scale);
