...
int main(int argc, char** argv) {
    ...
    if (opt_flag.find("_mt") == std::string::npos) {
        opt_flag += "_mt";
    }
    ...
    {
        #pragma omp single
        std::cout << "Running C++ flash_attn2_" << opt_flag << ": T=" << T << ", d=" << d << ", M=" << M_bytes/1024 << "KiB, opt=" << opt_flag << std::endl
                  << "Running with " << omp_get_num_threads() << " OpenMP threads to " << out_dir << std::endl;
    }
