...
MatrixXfRM fa2_forward(const MatrixXfRM& Q, const MatrixXfRM& Kt, const MatrixXfRM& V, 
    ...
    double& runtime, double& t7, double& t8, double& t10, double& t11, double& others) {
    int I = M_bytes / (4 * d * 4);
    int J = std::min(I, d);
    if (I < 1) I = 1;
    if (J < 1) J = 1;
    ...
    for (int i =0; i < Tr; i++){
        ...
        for (int j = 0; j < Tc; j++){
            ...
            t7 += std::chrono::duration<double>(std::chrono::steady_clock::now() - tt).count();

            // -------------Line 8----------
            tt = std::chrono::steady_clock::now();
            MatrixXfRM S = Q_i * Kt_j;
            S *= scale;
            t8 += std::chrono::duration<double>(std::chrono::steady_clock::now() - tt).count();
            ...
            t10 += std::chrono::duration<double>(std::chrono::steady_clock::now() - tt).count();
            // -------------Line 11------------
            tt = std::chrono::steady_clock::now();
            auto O_block = O.middleRows(row_start, cur_I);
            O_block.array().colwise() *= alpha.array();
            O.middleRows(row_start, cur_I) += (P * V_j);
            t11 += std::chrono::duration<double>(std::chrono::steady_clock::now() - tt).count();
    ...
    runtime = std::chrono::duration<double>(t1 - t0).count();
    others = runtime - (t7 + t8 + t10 + t11);
    if (others < 0.0) others = 0.0;
    return O;
}
...
int main(int argc, char** argv) {
    ...
    std::string out_dir = "outputs/fa2_cpp_profile_" + opt_flag;
    ...
    for (int i = 0; i < num_testsets; ++i) {
        ...
        (void)fa2_forward(Q, Kt, V, T, d, M_bytes, scale, runtime, t7, t8, t10, t11, others);

        std::cout << "---------------------------------------\n";
        std::cout << "Runtime " << i << ": " << std::fixed << std::setprecision(4) << runtime << " seconds." << std::endl;
        std::cout << "Line 7 " << std::fixed << std::setprecision(4) << t7 << " s ";
        std::cout << "Line 8 " << std::fixed << std::setprecision(4) << t8 << " s ";
        std::cout << "Line 10 " << std::fixed << std::setprecision(4) << t10 << " s ";
        std::cout << "Line 11 " << std::fixed << std::setprecision(4) << t11 << " s ";
        std::cout << "Others " << std::fixed << std::setprecision(4) << others << " s ";
        std::ofstream f(log_path, std::ios::app);
        f << i << "," << T << "," << d << "," << M_bytes << "," << std::fixed << std::setprecision(6) << runtime << "," << t7 << "," << t8 << "," << t10 << "," << t11 << "," << others << "\n";
    }
    return 0;
}